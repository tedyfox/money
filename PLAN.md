...
- Страница `/analytics`: бюджет месяца, обязательные расходы (localStorage), разбивка по неделям и категориям, список всех записей.
- Бюджет и обязательные расходы редактируются инлайн прямо на странице (хранятся в localStorage).
- **Редактирование и удаление записей** — каждая строка в «Все записи» имеет кнопки `✎` / `×`. На тапе `✎` строка разворачивается в форму (сумма, валюта, категория, дата, комментарий). Сохранение — Enter или кнопка, отмена — Escape или `✕`. Удаление мгновенное с optimistic rollback при ошибке сети. API: `PUT /api/expenses/[id]`, `DELETE /api/expenses/[id]`.
